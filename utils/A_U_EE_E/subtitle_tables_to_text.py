...
def process_table(input_file, output_file):
    ...
    with open(output_file, 'w', newline='', encoding='utf-8') as csvfile:
        ...
        for row in ws.iter_rows(min_row=2, values_only=True):  # Assuming the first row is a header
            # Assuming row format: [start_hms, start_ms, end_hms, end_ms, subtitle]
            start_hms = str(row[0]).split(':')  # start time hours:minutes:seconds
            ...
            start_time = time_to_milliseconds(start_hms[0], start_hms[1], start_hms[2], start_ms)
            end_time = time_to_milliseconds(end_hms[0], end_hms[1], end_hms[2], end_ms)
