...
class CallHistoryHandler:
    ...
    async def handle_fetch_call_history(self, websocket, data, aes_key):
        """
        ...
        user_id, payload = await self._validate_jwt(websocket, data, aes_key, msg_type)
        if user_id is None:
            return
        ...
        try:
            ...
            logger.error(
                f"Error fetching call history for user {user_id}: {exc}")
    ...
    def _serialize_entry(self, entry: dict) -> dict:
        ...
        entry_copy["started_at"] = entry_copy.get(
