...
def test_refresh_token_lifecycle():
    ...
    save_refresh_token(user_id, jti1, hash1, expires)
    assert find_valid_token(jti1, hash1)["jti"] == jti1

    # Revoke it via revoke_previous_token
    prev = revoke_previous_token(user_id, jti2)
    assert prev == jti1
    assert find_valid_token(jti1, hash1) is None

    # Now save a new one and revoke by id
    save_refresh_token(user_id, jti2, hash2, expires)
    revoke_token(jti2)
    assert find_valid_token(jti2, hash2) is None
